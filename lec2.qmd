...
```{r}
#| include: false
knitr::opts_chunk$set(fig.path = "assets/figs/lec2/")
```
...
- 📱 课程微信群：老师与助教会尽快回复
- 💻 GitHub 课程仓库的 Issues 区：<https://github.com/hcp4715/psybayesian/issues>
...
| 所有 (似然 × 先验) 的总和 | **证据 (evidence)**，即归一化因子 | $P(D)$ |
...
> 对应 Python 实现见 `lec2.py`：`scipy.stats.binom.pmf(y, n, p)`。
...
本讲的示例均有对应的 **Python 实现**，整理在课程根目录的 `lec2.py` 中：
...
> 运行方式：`python lec2.py`（需已安装 pandas、numpy、matplotlib、scipy；建议用和鲸平台 `pyBayesian` 环境或本地 conda 环境）。